...
iterations = [1, 5]
patche_sizes = [1, 2, 4, 7, 14, 28]
print('Launching on %d nodes' % (n_tries * len(iterations) * len(patche_sizes)))

for i in range(n_tries):
    for n_it in iterations:
        for ps in patche_sizes:
            if n_it == 1:
                lr = 0.001
            else:
                lr = 0.002
            args_str = '--n_it %d --seed %s --ps %d --lr %f' % (n_it, i, ps, lr)
            name = 'sink_' + str(n_it)
